...
### The C++ Programming
The first step was to create a connection link between the Wiimote and a bluetooth reciever connected to the ZedBoard.
...
The goal was to use the Wiimote acceleration (or movement) to control the robotic arm the desired way.
...
main.cpp
  WiimoteBtns.cpp
    Reads and processes wiimote events and sets the desired speed and angle values
  RoboticArm.cpp
    Controls the inputs sent to the registers that hold the inputs to the logic design that controls the servos

The code written for this control is on my Github [Insert GitHub Link].
...
Caption: The video demonstrating the operation and control of the robotic arm.
