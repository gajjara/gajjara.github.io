...
Also note that this is not a academic document on this research project shown here and should not be considered as such. 
...
In order to image collagen, transmission electron microscopy is used. The issue with transmission electron microscopy is that it tends to destroy samples after imaging; thus preventing _in vitro_ imaging. Our goal with this research is to prove to the scientific community interested in collagen that fluorescence microscopy is a viable alternative to electron microscopy and can allow _in vitro_ imaging of collagen. This live imaging of collagen can allow the further understanding of the functionality and properties of this biomolecule. This further understanding can unlock further biomedical advances.

**Summary:**
Overall, using the optical simulations that I devised, we were able to generate synthetic to train a convolutional neural network to 70% accuracy for the detection of collagen monomer orientation. Thus demonstrating we were successfully able to show that fluorescence microscopy can allow _in vitro_ imaging of collagen. As a side project to this research, I also was able to initially develop an algorithm to detect the orientation of collagen fibrils; which could be potentially used in a further development from this research. As stated above, this research has potential to unlock biomedical advances. A research article is currently in the process of publication on this research.
...
In order to use fluorescence microscopy for collagen, we decided to attach fluorophores onto collagen monomers, and determine their spatial orientation. This allows us to demonstrate the technique, and the spatial orientation can tell us important information about the monomers. The challenge with imaging the monomers is that since these monomers are at most 300nm long <a href="https://jcs.biologists.org/content/joces/120/12/1955.full.pdf/"> (source)</a>, and with one of our cameras we can only achieve a resolution of 110nm per pixel; we encounter a sampling problem as we are approaching the diffraction limits of the microscope. If two fluorophores can attach to two ends on a monomer, then enough fluorescence can occur that enough pixels with enough intensity can indicate a monomer orientation. A challenge is that two fluorophores may not always attach to a monomer (thus there will be double fluorophore and single fluorophore monomers when we image the monomers). Another is that noise, from the camera sensor, light scattering, and random autofluorescence in the sample can cause certain pixels to cause error in the detection or orientation or single fluorophore monomers. Therefore, it is difficult to use iterative techniques. For this reason, a machine learning model is later developed for the orientation detection or single fluorophore monomers. 

Due to the susceptibility of noise in the imaging of these monomers, we wanted to simulate the optical system to obtain further understanding of the fluorescence microscopy system, and to generate synthetic data consistent with real imaging data. One of my primary roles on this project was to determine this simulation in MATLAB. Along with helping with the development of the orientation detection programs, and the writing of a research article in process of publication on this work.
...
To simulate the optics system, I used spectrum on the spectral irradiance from the light source, transmission and reflection in the filter set, the emission and excitation spectrum of the fluorophore that we use, and the quantum efficiency (tells us the probability of photons that turn into electrons). The spectrum were obtained from manufacturers of the equipment we used.

Along with the absorption and scattering coefficients of the fluorophore, a solid angle calculation of the objective lens, the photon energy equation, the fluorophore quantum yield (efficiency of photon emission from photon absorption), and the fluorophore lifetime (the time of fluorophore excitation), I was able to conduct a simulation on the amount of photons from emission (green light photons) and photons from scattering (blue light photons) that enter the camera sensor.
...
Considering the dark current (the baseline light background noise) noise and the readout noise (sensor noise) in the camera, I was able to get a good estimate of the total noise we receive in imaging.
...
Caption: The spectrum calculated from the optics simulation.

<img src = "images/collagen_opticssim_3.png?raw=true"/>

Caption: The numerical values calculated from the optics simulation.

The code for this portion of this project is on my <a href="https://github.com/gajjara/CollagenCode/tree/master/MonomerWork">github</a>.
...
Using the values of signal and noise I calculated, we were able to generate a set of 20000 synthetic images. This set of images were used to train a convolutional neural network (CNN) that determined the orientation of monomers within 15 degrees or determined when there was a single fluorophore monomer.
...
Despite achieving an accuracy of 70%, the confusion matrix shows that even in an inaccurate detection, there is only an error of 15 degrees. Thus we are able to show from the CNN that despite random noise causing error for iterative methods of detection, we are able to detect the orientation of monomers (or single fluorophore monomers).

Again, credit goes to Rodrigo Alzola and Rickard Stureborg for their primary role in the development of this neural network.

### Fibril Orientation Initial Development
**Back Story**

Outside of this project, but still related, I attempted to detect the orientation of collagen fibrils. There is the potential that these collagen fibrils can be imaged alongside monomers, further increasing the potential benefits of this research.

Alongside with fluorescence microscopy, we can use differential interference contrast (DIC) imaging, a form of phase-contrast imaging to obtain images of fibrils. A challenge with these images is that it is difficult to separate signal and noise components in these images. Another challenge is also that when these fibrils are imaged, they tend to be a few pixels wide. A synthetic image below demonstrates these challenges.
...
The code for this portion of this project is on my <a href="https://github.com/gajjara/CollagenCode/tree/master/FibrilOrientation">github</a>.
...
As it can be seen that most elements of the fibrils can be detected, but there are still some missing parts of fibrils that are not detected. There is some false positives present, especially in the image showing the repetition of the algorithm. However, this algorithm shows promise and could be used or developed more in the future.
...
From our work we know that visualizing monomers of collagen with fluorescence microscopy and determining their orientation is not always precise as the process is very delicate and is affected by many factors. Such factors include noise from the scattering of light in a collagen sample, noise in the camera system, potential autofluorescence in a sample, the lack of fluorophores attaching to two ends in a monomer, and any mishaps in the making of a collagen sample.

Nonetheless, in many cases it is possible to determine the orientation. These approaches are useful for understanding if there is an alignment of monomers in, for example, fibril formation. Visualizing collagen monomers in the presence of a fibril and determining if there is a particular orientation in the attachment process in real time is a potential continuation of our work.
...
Again, credit goes to Rodrigo P. Alzola, Jose Delpiano, Rickard Stureborg, Seyed M. Siadat, Professor Jeffrey W. Ruberti, and Professor Charles A. DiMarzio.
