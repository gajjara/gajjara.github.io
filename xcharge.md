...
Caption: The design for the piezoelectrics, in which the blue highlights 3D printed parts, while the red highlights the placement of piezoelectrics.
...
Caption: A simple electrical circuit for output voltage regulation from a peltier or piezoelectric (the left most circuit in the diagram). The voltage divider was later removed, and a diode was added to the wire leading to the battery to prevent charge transfer from the battery to the piezoelectrics or peltier tiles.
...
As shown, this data is not promising from an energy standpoint. From 6 hours of gameplay, the gamepad can only be charged by 0.084aH (given that there is no charge loss in the battery). This was not necessarily surprising to us, as there is a lack of high charge potential in the piezoelectric and peltier materials that we used. Nonetheless, our scope of our project is prove that this concept is feasible, given improvements in the energy potential of piezoelectrics and peltier tiles.

We then collected data on the potential amount of events we would receive from the Xbox controller. For this, I played 70 minutes of the FIFA 18 game, as this was a reasonable simulation of the typical button, joystick, and trigger events seen from gameplay.

Using a Raspberry Pi with the evdev Python library, I was able to store a text file of all the button events received during the 70 minutes of gameplay.
...
**Reflection**
Our project was not necessarily successful in the sense that we could not charge the Xbox controller battery by any sign giant amount. However, we successfully proved that it is possible for microenergy elements to be present in a game controller, and given potential materials improvements, and optimization of the circuitry I devised for regulating the output voltages from the piezoelectrics and peltiers, it is possible in the future that a gamepad could be made that charges itself.

This project is part of a greater push for the utilization of new energy sources, and as stated above, microenergy generation is a part of that push. If microenergy systems are used in anything that could generate energy (i.e. door hinges, water pipes) there could be a reduced usage and reliability on large power grids. This project, while not necessarily impactful on a large scale, is part of that push, and will allow at least one of us in the group to develop successful (from an energy generation perspective) microenergy systems.
